...
for i in range(sheetRead.nrows):
    for p in range(sheetRead.ncols):
        if sheetRead.cell_value(i,p) != "": #If cell is empty don't copy any data since you can only write to each cell once
...
for i in range(dataRead.nrows):
    for p in range(dataRead.ncols):
        if dataRead.cell_value(i,p) != "": #If cell is empty don't copy any data since you can only write to each cell once
...
for i in range(sheetRead.nrows-1):
    ...
    try:
        sheetWrite.write(i + 1, 8, contentssplit[Titles.index("precipIntensityMax")])
    except ValueError:
        sheetWrite.write(i + 1, 8, "")

    #PrecipitationProbabilityHigh
    try:
        sheetWrite.write(i + 1, 9, contentssplit[Titles.index("precipProbability")])
    except ValueError:
        sheetWrite.write(i + 1, 9, "")

    #TempratureHigh
    try:
        sheetWrite.write(i + 1, 10, contentssplit[Titles.index("temperatureHigh")])
    except ValueError:
        sheetWrite.write(i + 1, 10, "")

    #TempratureLow
    try:
        sheetWrite.write(i + 1, 11, contentssplit[Titles.index("temperatureLow")])
    except ValueError:
        sheetWrite.write(i + 1, 11, "")

    #ApparentTempratureHigh
    try:
        sheetWrite.write(i + 1, 12, contentssplit[Titles.index("apparentTemperatureHigh")])
    except ValueError:
        sheetWrite.write(i + 1, 12, "")

    #ApparentRempratureLow
    try:
        sheetWrite.write(i + 1, 13, contentssplit[Titles.index("apparentTemperatureLow")])
    except ValueError:
        sheetWrite.write(i + 1, 13, "")

    #DewPoint
    try:
        sheetWrite.write(i + 1, 14, contentssplit[Titles.index("dewPoint")])
    except ValueError:
        sheetWrite.write(i + 1, 14, "")

    #Humidity
    try:
        sheetWrite.write(i + 1, 15, contentssplit[Titles.index("humidity")])
    except ValueError:
        sheetWrite.write(i + 1, 15, "")

    #Pressure
    try:
        sheetWrite.write(i + 1, 16, contentssplit[Titles.index("pressure")])

        #Find the indicies of all things titled pressure
        ...
        #Iterate through the indicies and replace highest if i2 is higher than highest and lower if i2 is lower than lowest
        for i2 in indices:
    ...
    #WindSpeed
    try:
        sheetWrite.write(i + 1, 18, contentssplit[Titles.index("windSpeed")])
    except ValueError:
        sheetWrite.write(i + 1, 18, "")

    #WindGust
    try:
        sheetWrite.write(i + 1, 19, contentssplit[Titles.index("windGust")])
    except ValueError:
        sheetWrite.write(i + 1, 19, "")

    #WindBearing
    try:
        sheetWrite.write(i + 1, 20, contentssplit[Titles.index("windBearing")])
    except ValueError:
        sheetWrite.write(i + 1, 20, "")

    #CloudCover
    try:
        sheetWrite.write(i + 1, 21, contentssplit[Titles.index("cloudCover")])
    except ValueError:
    ...
    #Visibility
    try:
        sheetWrite.write(i + 1, 23, contentssplit[Titles.index("visibility")])
    except ValueError:
        sheetWrite.write(i + 1, 23, "")

    #Ozone
    try:
        sheetWrite.write(i + 1, 24, contentssplit[Titles.index("ozone")])
    except ValueError:
        sheetWrite.write(i + 1, 24, "")

    #Moon Phase
    try:
        sheetWrite.write(i + 1, 25, contentssplit[Titles.index("moonPhase")])
    except ValueError:
